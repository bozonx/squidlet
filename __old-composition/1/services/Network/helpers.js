 ...
 * @param message
 */
export function encodeNetworkMessage(message) {
 ...
 * @param data
 */
export function decodeNetworkMessage(data) {
    const completeRouteLength = data[9];
    const [parsedArr, completeRouteStartIndex] = deserializeStringArray(data, 10, 2);
    const [completeRoute, lastIndex] = deserializeStringArray(data, completeRouteStartIndex + 1, completeRouteLength);
    return {
        TTL: data[0],
